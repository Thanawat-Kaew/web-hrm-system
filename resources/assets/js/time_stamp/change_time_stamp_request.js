$(function(){
	$('.view-data-request-forget-to-time').click(function(){
		msg_waiting()
		var	id = $(this).data('id');
		//console.log(id);
		$.ajax({
			...
			data: {'method' : 'getViewDataRequestForgetToTime',
			'id'	    : id
		},
	...
	msg_waiting()
	$('.btn-confirm-data-request-forget-to-time').click(function(){  // กด อนุมัติ
		//alert("confirm");
		var id = $(this).data('id');
		//console.log(id);
		//alert(id);
		$.ajax({
			headers: {'X-CSRF-TOKEN': $('input[name=_token]').attr('value')},
			type: "POST",
			url: $('#confirm-data-request-forget-to-time').data('url'),
			data: {'id'	    : id
		}
	});
		Swal.fire({
	...
	$('.btn-cancel-data-request-forget-to-time').click(function(){  // กดปฏิเสธ
		var id = $(this).data('id');
		//alert(id);
		Swal.fire({
			title: 'คุณแน่ใจหรือไม่?',
				...
				Swal.fire({
					...
					if(result.value !== ''){
						...
						$.ajax({
							headers: {'X-CSRF-TOKEN': $('input[name=_token]').attr('value')},
							type: "POST",
							url: $('#cancel-data-request-forget-to-time').data('url'),
							data:{'id'	           : id,
